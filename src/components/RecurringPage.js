...
export default function RecurringPage({ showToast, onRefresh }) {
  ...
  return (
    // FIX: use a CSS class (.recurring-page-wrap) instead of inline style
    // so the desktop padding override in index.css can target it reliably.
    <div className="recurring-page-wrap">
      <h1 style={S.title}>Recurring Transactions</h1>
      <p style={S.sub}>
        Automate salary, rent, EMI and more — runs automatically at midnight
        every day.
      </p>
      ...
      {dueCount > 0 && (
        ...
        >
          ...
          <span>
            ...
            {dueCount > 1 ? "s are" : " is"} due. Click "Run now" to trigger
            immediately.
          </span>
        </div>
      )}
        ...
        <div style={S.card}>
          ...
          {accounts.length === 0 ? (
            <div style={S.noAccWarn}>
              ⚠ No accounts found. Create an account first before adding a
          ...
          {loadingCats ? (
            ...
            <div style={S.noCatWarn}>
              ⚠ No {form.type.toLowerCase()} categories found. Go to the{" "}
              <strong>Categories page</strong> to add some first.
            </div>
          ) : (
            <select
              style={S.select}
          ...
          >
            ...
            on the frequency. You can also click "Run now" to trigger manually.
          </div>
        </div>
      ...
      {confirmId && (
        <div style={S.confirmOverlay}>
          <div style={S.confirmBox}>
            ...
            >
              This stops all future automatic runs. Past transactions are not
              affected.
            </p>
      ...
      <style>{`
        ...
        .recurring-page-wrap { padding: 16px; min-height: 100vh; color: #E8EDF5; font-family: 'DM Sans', 'Segoe UI', sans-serif; }
        @media (min-width: 900px) { .recurring-page-wrap { padding: 36px 40px; } }
      `}</style>
    </div>
  );
}
